...
description: Build and rebuild ONE course module's map — its subtree of the fused course map, leaves as paged notebooks extracted faithfully with machine-checked coverage and a human-gated plan. Use for a single module, or when build-course-map delegates a module to you.
---
...
3. **On approval**: record it, then generate —
   ...
   PYTHONPATH=<base>/tools python3 -m revision_atlas.build <module_dir> --mindmaps <out> \
       [same pass flags]
   PYTHONPATH=<base>/tools python3 -m revision_atlas.course --module <module_dir> --mindmaps <out>
   ```
   The last command renders the fused course map (a course of one module). The
   notebooks' back-links point at `mindmaps/index.html`, so this render is what
   makes them land somewhere.
